...
export const endpoints = {
  ...
  quizzes: { method:'GET',path:'/quizzes',roles:student,feature:'Student quiz',summary:'Danh sách quiz đã xuất bản',query:s.pagination,rpc:'platform_quizzes',action:'list',response:'QuizList' },
  quiz: { method:'GET',path:'/quizzes/{id}',roles:student,feature:'Student quiz',summary:'Đề quiz không chứa đáp án hoặc giải thích',rpc:'platform_quizzes',action:'get',response:'StudentQuiz' },
  submitQuiz: { method:'POST',path:'/quizzes/{id}/submissions',roles:student,feature:'Student quiz',summary:'Chấm trên server và lưu kết quả; requestId chống nộp trùng',body:s.submissionInput,rpc:'platform_quizzes',action:'submit',response:'Attempt' },
  attempts: { method:'GET',path:'/quizzes/submissions',roles:student,feature:'Student quiz',summary:'Lịch sử làm bài của chính Student',query:s.pagination,rpc:'platform_quizzes',action:'attempts',response:'AttemptList' },
  attempt: { method:'GET',path:'/quizzes/submissions/{id}',roles:student,feature:'Student quiz',summary:'Đọc điểm và lời giải của lần nộp bài',rpc:'platform_quizzes',action:'attempt',response:'Attempt' },
  lectureQuizzes: { method:'GET',path:'/lecture/quizzes',roles:staff,feature:'Lecture quiz',summary:'Danh sách quiz do mình soạn; Admin xem toàn bộ',query:s.pagination,rpc:'platform_quizzes',action:'list',response:'QuizList' },
  createQuiz: { method:'POST',path:'/lecture/quizzes',roles:staff,feature:'Lecture quiz',summary:'Soạn quiz với đáp án và giải thích',body:s.quizInput,rpc:'platform_quizzes',action:'create',status:201,response:'Quiz' },
  lectureQuiz: { method:'GET',path:'/lecture/quizzes/{id}',roles:staff,feature:'Lecture quiz',summary:'Đọc đề và đáp án trong phạm vi sở hữu',rpc:'platform_quizzes',action:'get',response:'Quiz' },
  updateQuiz: { method:'PUT',path:'/lecture/quizzes/{id}',roles:staff,feature:'Lecture quiz',summary:'Sửa quiz và tăng revision',body:s.quizUpdate,rpc:'platform_quizzes',action:'update',response:'Quiz' },
  deleteQuiz: { method:'DELETE',path:'/lecture/quizzes/{id}',roles:staff,feature:'Lecture quiz',summary:'Xóa mềm quiz đã ngừng xuất bản',body:s.revisionInput,rpc:'platform_quizzes',action:'delete',response:'Quiz' },
  publishQuiz: { method:'POST',path:'/lecture/quizzes/{id}/publish',roles:staff,feature:'Lecture quiz',summary:'Xuất bản đề quiz',body:s.revisionInput,rpc:'platform_quizzes',action:'publish',response:'Quiz' },
  archiveQuiz: { method:'POST',path:'/lecture/quizzes/{id}/archive',roles:staff,feature:'Lecture quiz',summary:'Ngừng nhận bài cho quiz',body:s.revisionInput,rpc:'platform_quizzes',action:'archive',response:'Quiz' },
  ...
  audit: { method:'GET',path:'/admin/audit',roles:admin,feature:'Admin audit',summary:'Nhật ký thao tác role, tài liệu và quiz',query:s.pagination,rpc:'platform_admin',action:'audit',response:'AuditList' },
  analytics: { method:'GET',path:'/admin/analytics',roles:admin,feature:'Admin analytics',summary:'Số user, tài liệu published, review, roadmap và lượt quiz',rpc:'platform_admin',action:'analytics',response:'Analytics' },
