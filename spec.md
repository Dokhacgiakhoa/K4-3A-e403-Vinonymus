...
| 17/9 (sau CP4) | Nối K.AI Helpdesk với API RAG thật; thêm agent chọn `search/clarify/refuse/handoff/chat`, tài liệu kiến trúc Markdown và kiểm tra validation/logging bằng Gemini 3.5 Flash-Lite | Thay widget gán cứng của dự án nền; thay đổi này không sửa Quality Bar đã khóa cho Planner |
...
| 17/9 (sau phản hồi người dùng) | Nối JWT của backend vào AI Helpdesk, giữ nguyên `Visitor/Member/Lecture/SuperAdmin`; chỉ suy ra quyền dùng tính năng học tập | Role được xác thực server-side; thiếu token, role lạ hoặc lỗi auth fail closed về `Visitor`; chưa thay đổi corpus, embedding, schema, retrieval hoặc Quality Bar §7 |
| 18/9 (sau phản hồi người dùng) | Chuẩn hóa riêng 34 Markdown VLearn Day 1–15 (bỏ 3 bản trùng, 1 bản hỏng), thêm `audience: learning`, RLS và retrieval theo role; Git bỏ qua cả nguồn lẫn bản dẫn xuất | Thay ba document mẫu bằng corpus bài học thật nhưng không công khai tài liệu VLearn; migration/sync production còn chờ áp dụng trên Supabase; không đổi Quality Bar §7 |
| 18/9 (sau phản hồi người dùng) | AI Helpdesk lưu session memory cục bộ theo guest/user và nhận `learner_context` rút gọn từ roadmap; server loại context của `Visitor` | Cá nhân hoá cách giải thích theo lộ trình mà không trộn roadmap vào memory hoặc dùng context làm nguồn kiến thức; không đổi Quality Bar §7 |
| 18/9 (sau phản hồi người dùng) | Chuyển nguồn chính của Helpdesk session memory sang Supabase (`chat_sessions`, `chat_messages`), dùng guest cookie `HttpOnly` hoặc backend `user_id`; localStorage chỉ fallback | Có memory bền vững và đồng bộ cho tài khoản mà không cho client tự khai owner; migration `0016` cần áp dụng trên Supabase; không đổi Quality Bar §7 |
